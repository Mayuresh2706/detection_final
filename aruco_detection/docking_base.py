...
#         # Tuning
#         self.stop_dist = 0.10
#         self.lateral_threshold = 0.05  # rvec_z threshold — on centre line
#         self.yaw_threshold = 0.05      # rvec_y threshold — facing square (~3°)
...
#             self.get_logger().info('Activated — Step 1: getting on centre line...')
#         else:
#             self.get_logger().info('Deactivated.')
#             self.stop_robot()
...
#         # ── STEP 1: get on centre line using rvec_z ──
#         # rvec_z > 0 → robot is to the left  → rotate right (negative angular.z)
#         # rvec_z < 0 → robot is to the right → rotate left  (positive angular.z)
#         # Drive forward slowly while rotating to physically move laterally
#         if self.state == 'fix_lateral':
#             if marker_age > self.marker_timeout:
#                 self.cmd_pub.publish(Twist())
#                 self.get_logger().warn('Step 1: waiting for marker...')
#                 return

#             rvec_z = self.last_marker_rvec_z

#             if abs(rvec_z) < self.lateral_threshold:
#                 self.cmd_pub.publish(Twist())
#                 self.get_logger().info(
#                     f'Step 1 done! rvec_z={rvec_z:.3f} — Step 2: fixing yaw...'
#                 )
#                 self.state = 'fix_yaw'
#                 return
...
#             cmd = Twist()
#             cmd.linear.x = 0.03
#             # rvec_z positive = left of line = rotate right = negative angular.z
#             cmd.angular.z = max(-self.ang_speed,
#                                 min(self.ang_speed, -2.0 * rvec_z))
#             self.cmd_pub.publish(cmd)
#             self.get_logger().info(
#                 f'Step 1: rvec_z={rvec_z:.3f} '
#                 f'({math.degrees(rvec_z):.1f}°)'
#             )

#         # ── STEP 2: fix yaw using rvec_y ──
#         # Rotate in place only, no forward movement
#         elif self.state == 'fix_yaw':
#             if marker_age > self.marker_timeout:
#                 self.cmd_pub.publish(Twist())
#                 self.get_logger().warn('Step 2: waiting for marker...')
#                 return

#             rvec_y = self.last_marker_rvec_y

#             if abs(rvec_y) < self.yaw_threshold:
#                 self.cmd_pub.publish(Twist())
#                 self.get_logger().info(
#                     f'Step 2 done! rvec_y={rvec_y:.3f} — Step 3: driving in...'
#                 )
#                 self.aligned_count = 0
#                 self.state = 'visual_servo'
#                 return
...
#             cmd.linear.x = 0.0
#             cmd.angular.z = max(-self.ang_speed,
#                                 min(self.ang_speed, 2.0 * rvec_y))
#             self.cmd_pub.publish(cmd)
#             self.get_logger().info(
#                 f'Step 2: rvec_y={rvec_y:.3f}rad '
#                 f'({math.degrees(rvec_y):.1f}°)'
#             )

#         # ── STEP 3: visual servo — drive in while keeping centred ──
#         elif self.state == 'visual_servo':
#             if marker_age > self.marker_timeout:
#                 if 0.0 < self.last_marker_z < 0.35:
#                     self.drive_distance = max(0.0, self.last_marker_z - self.stop_dist)
...
#                         'Step 3: marker lost too far out — waiting...'
#                     )
#                 return
...
#         # ── STEP 4: odom straight drive for last bit ──
#         elif self.state == 'odom_drive':
#             dist_traveled = math.sqrt(
#                 (self.current_x - self.start_x) ** 2 +
#                 (self.current_y - self.start_y) ** 2
...
class DockingBase(Node):
    def __init__(self, node_name, dock_marker_id):
        ...
        self.last_marker_time = None
        self.marker_timeout = 0.3
        self.marker_lost_timeout = 2.0  # give up after 2s blind
    ...
    def drive_callback(self):
        ...
        if self.state == 'fix_lateral':
            if marker_age > self.marker_timeout:
                if marker_age > self.marker_lost_timeout:
                    # Been blind too long — stop and wait for reacquisition
                    self.cmd_pub.publish(Twist())
                    self.get_logger().warn(
                        'Step 1: marker lost too long — stopping.'
                    )
                    return
                # Brief loss — keep driving on last known rvec_z
                rvec_z = self.last_marker_rvec_z
                cmd = Twist()
                cmd.linear.x = 0.03
                cmd.angular.z = max(-self.ang_speed,
                ...
                self.cmd_pub.publish(cmd)
                self.get_logger().warn(
                    f'Step 1: marker lost — last known rvec_z={rvec_z:.3f}'
                )
                return
            ...
            self.cmd_pub.publish(cmd)
            self.get_logger().info(
                f'Step 1: rvec_z={rvec_z:.3f} ({math.degrees(rvec_z):.1f}°)'
            )
        ...
        # ── STEP 2: fix yaw using rvec_y ──
            ...
            if marker_age > self.marker_timeout:
                if marker_age > self.marker_lost_timeout:
                    self.cmd_pub.publish(Twist())
                    self.get_logger().warn(
                        'Step 2: marker lost too long — stopping.'
                    )
                    return
                # Brief loss — keep rotating on last known rvec_y
                rvec_y = self.last_marker_rvec_y
                cmd = Twist()
                cmd.linear.x = 0.0
                cmd.angular.z = max(-self.ang_speed,
                ...
                self.cmd_pub.publish(cmd)
                self.get_logger().warn(
                    f'Step 2: marker lost — last known rvec_y={rvec_y:.3f}'
                )
                return
            ...
            self.cmd_pub.publish(cmd)
            self.get_logger().info(
                f'Step 2: rvec_y={rvec_y:.3f}rad ({math.degrees(rvec_y):.1f}°)'
            )
